  ...
  return (
    <div className="glass rounded-xl h-full overflow-hidden">
      ...
      {conceptDetail.wormholes.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-galaxy-pink mb-3 flex items-center">
            <SparklesIcon className="h-5 w-5 mr-2" />
            Wormholes
          </h3>
          <p className="text-galaxy-white/60 text-xs mb-3">
          ...
          <div className="space-y-2">
            {conceptDetail.wormholes.map((wormhole, index) => (
              <div
                key={index}
                className="concept-card p-3 rounded-lg cursor-pointer"
                onClick={() => {
                  // TODO: Navigate to wormhole target
                }}
              >
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-medium text-galaxy-white text-sm">
                ...
                <p className="text-galaxy-white/70 text-xs line-clamp-2">
      ...
      {/* Neighbors Section */}
      <div>
        <h3 className="text-lg font-semibold text-galaxy-white mb-3">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-galaxy-white/50">
                    {neighbor.edge_type}
                  </span>
                  <span className="text-xs text-galaxy-cyan">
              ...
              {neighbor.concept.category && (
                <span className="inline-block mt-2 px-2 py-1 bg-galaxy-purple/20 text-galaxy-purple text-xs rounded-full">
                  {neighbor.concept.category}
                </span>
              )}
            </div>
